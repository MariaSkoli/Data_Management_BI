...
Connection=odbcDriverConnect('driver={SQL Server};server=LAPTOP-U0DI431G;database=E-Properties;trusted_connection=true')
...
WHILE @codeRegion <= 20
	...
	IF ( @codeRegion in (SELECT r.id as Region FROM Region R  INNER JOIN Property P ON P.Id_Region=R.ID INNER JOIN Estimation E ON E.Id_Property=P.ID WHERE YEAR(E.Date)= 2020))
		...
		SET @EstPop=( SELECT sum(r.Population)/count(R.ID) FROM Region R INNER JOIN Property P ON P.Id_Region=R.ID INNER JOIN Estimation E ON E.Id_Property=P.ID WHERE R.ID=@codeRegion and YEAR(E.Date)= 2020)
...
k=sqlQuery(Connection, "select  A.ID
		...
		FROM (select * from dbo.TemporaryTable as T) as A , ( select count(E.ID) AS TotalEst from Estimation AS E WHERE YEAR(E.Date)=2020) as B , 
			( select SUM( R.Population)  AS TotalPop from Region R INNER JOIN Property P ON P.Id_Region=R.ID INNER JOIN Estimation E ON E.Id_Property=P.ID	WHERE YEAR(E.Date)= 2020 ) as C")
k